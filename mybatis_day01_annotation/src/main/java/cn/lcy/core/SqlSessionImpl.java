...
public class SqlSessionImpl implements SqlSession {
     ...
     * @return
     */
    public <T> List<T> queryAll(String sql, String resultType) {
        //获取连接
    ...
    public <E> E getMapper(Class<E> clazz) throws Exception {
        ...
        E proxy1 = (E) Proxy.newProxyInstance(
                ...
                (Object proxy, Method method, Object[] args) -> {
                    ...
                    //判断是否有注解
                    boolean flag = method.isAnnotationPresent(QueryAll.class);
                    if (flag) {
                        QueryAll annotation = method.getAnnotation(QueryAll.class);
                        String id = annotation.id();
                        if (id.equals(name)) {
                            String sql = annotation.sql();
                            String resultType = annotation.resultType();
                            List list = queryAll(sql, resultType);
                            return list;
                        }
                    }
                    return null;
